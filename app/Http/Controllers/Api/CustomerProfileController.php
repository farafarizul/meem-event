...
class CustomerProfileController extends Controller
{
    ...
    public function show(Request $request): JsonResponse
    {
        ...
        if ($upstream->successful() && ($body['success'] ?? false) && isset($body['data'])) {
            ...
            //for testing, make a random value for sss_balance between 0.01 and 10000 to test the progress detail. you can change the value to test different threshold.
            $sss_balance = rand(1, 10000) / 100;
        ...
        if($upstream->successful() && ($body['success'] ?? false) && isset($body['data']['gss_balance'])) {
            $log_data['app_session'] = $request->input('app_session', 'unknown');
            $log_data = $body['data'];
