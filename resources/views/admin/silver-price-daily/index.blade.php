<x-app-layout>
    <x-slot name="header">Silver Price Daily</x-slot>
        ...
        <div class="col-xl-3 col-md-4 col-sm-6">
            <div class="card border-0 shadow-sm h-100">
                <div class="card-body d-flex align-items-center gap-3">
                    ...
                    <div>
                        <div class="fs-4 fw-bold text-success">{{ number_format($stats['total_positive']) }}</div>
                        <div class="text-muted small">Total Positive Days</div>
                    </div>
                </div>
            </div>
        </div>
        <div class="col-xl-3 col-md-4 col-sm-6">
            <div class="card border-0 shadow-sm h-100">
                <div class="card-body d-flex align-items-center gap-3">
                    ...
                    <div>
                        <div class="fs-4 fw-bold text-danger">{{ number_format($stats['total_negative']) }}</div>
                        <div class="text-muted small">Total Negative Days</div>
                    </div>
                </div>
            </div>
        </div>
        <div class="col-xl-3 col-md-4 col-sm-6">
            <div class="card border-0 shadow-sm h-100">
                <div class="card-body d-flex align-items-center gap-3">
                    ...
                    <div>
                        <div class="fs-4 fw-bold text-secondary">{{ number_format($stats['total_neutral']) }}</div>
                        <div class="text-muted small">Total Neutral Days</div>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="card border-0 shadow-sm mb-4">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route('admin.silver-price-daily.manual-sync') }}" class="row g-2 align-items-end">
                @csrf
                <div class="col-auto">
            ...
            <p class="text-muted small mt-2 mb-0">
                This will aggregate daily summary from <code>silver_price</code> records and call Groq AI for the selected date.
            </p>
        </div>
    </div>
    ...
    <div class="card border-0 shadow-sm">
        <div class="card-header bg-white">
            <h6 class="mb-0 fw-semibold"><i class="bi bi-table me-1"></i>Daily Silver Price Records</h6>
        </div>
        <div class="card-body">
            ...
            <div class="table-responsive">
                <table id="silver-price-daily-table" class="table table-hover align-middle w-100">
    ...
    @push('scripts')
    <script>
        var table = $('#silver-price-daily-table').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                url: '{{ route('admin.silver-price-daily.datatable') }}',
                type: 'GET',
                data: function (d) {
            ...
            columns: [
                ...
                { data: 'silver_price_date',     name: 'silver_price_date' },
